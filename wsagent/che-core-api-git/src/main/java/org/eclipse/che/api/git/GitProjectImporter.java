...
public class GitProjectImporter implements ProjectImporter {
    ...
    private void cloneRepository(GitConnection git, String remoteName, String url)
            throws ServerException, UnauthorizedException, URISyntaxException {
        final CloneParams params = CloneParams.create().withRemoteName(remoteName).withRemoteUri(url);
        git.clone(params);
    }

    private void addRemote(GitConnection git, String name, String url) throws GitException {
        git.remoteAdd(RemoteAddParams.create().withName(name).withUrl(url));
    }

    private void fetch(GitConnection git, String remote) throws UnauthorizedException, GitException {
        final FetchParams params = FetchParams.create().withRemote(remote);
        git.fetch(params);
    }
    ...
    private void fetchRefSpecs(GitConnection git, String remote, List<String> refSpecs)
            throws UnauthorizedException, GitException {
        final FetchParams params = FetchParams.create().withRemote(remote).withRefSpec(refSpecs);
        git.fetch(params);
    }

    private void checkoutCommit(GitConnection git, String commit) throws GitException {
        final CheckoutParams params = CheckoutParams.create().withName("temp")
                                                    .withCreateNew(true)
    ...
    private void checkoutBranch(GitConnection git,
        ...
        final CheckoutParams params = CheckoutParams.create().withName(branchName);
