...
public class MergeTest {
    ...
    public void testMerge(GitConnectionFactory connectionFactory) throws Exception {
        //given
        GitConnection connection = connectToGitRepositoryWithContent(connectionFactory, repository);
        connection.checkout(CheckoutParams.create().withName(branchName).withCreateNew(true));
        File file = addFile(connection, "t-merge", "aaa\n");

        connection.add(AddParams.create().withFilePattern(new ArrayList<>(Arrays.asList("."))));
        connection.commit(CommitParams.create().withMessage("add file in new branch"));
