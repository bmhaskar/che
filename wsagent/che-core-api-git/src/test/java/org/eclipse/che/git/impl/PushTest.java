...
public class PushTest {
    ...
    public void testSimplePush(GitConnectionFactory connectionFactory)
        ...
        remoteConnection.clone(CloneParams.create().withRemoteUri(connection.getWorkingDir().getAbsolutePath())
                                          .withWorkingDir(remoteConnection.getWorkingDir().getAbsolutePath()));
        addFile(remoteConnection, "newfile", "content");
        remoteConnection.add(AddParams.create().withFilePattern(Arrays.asList(".")));
        remoteConnection.commit(CommitParams.create().withMessage("Fake commit"));
        //when
        remoteConnection.push(PushParams.create()
        ...
        connection.checkout(CheckoutParams.create().withName("test"));
    ...
    public void testPushRemote(GitConnectionFactory connectionFactory)
        ...
        connection.commit(CommitParams.create().withMessage("Init commit."));
        //make push
        int branchesBefore = remoteConnection.branchList(LIST_LOCAL).size();
        //when
        connection.push(PushParams.create().withRefSpec(Arrays.asList("refs/heads/master:refs/heads/test"))
                                                 .withRemote(remoteRepo.getAbsolutePath())
                                                 .withTimeout(-1));
        //then
        int branchesAfter = remoteConnection.branchList(LIST_LOCAL).size();
    ...
    public void testWhenThereAreNoAnyRemotes(GitConnectionFactory connectionFactory) throws Exception {
        ...
        //when
        connection.push(PushParams.create());
    }
}
