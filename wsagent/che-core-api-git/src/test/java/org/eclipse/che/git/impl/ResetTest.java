...
public class ResetTest {
    ...
    public void testResetHard(GitConnectionFactory connectionFactory) throws Exception {
        ...
        connection.add(AddParams.create().withFilePattern(new ArrayList<>(Arrays.asList("."))));
        connection.commit(CommitParams.create().withMessage("add file"));
        //when
        connection.reset(ResetParams.create().withCommit("HEAD^").withType(ResetRequest.ResetType.HARD));
        //then
    ...
    public void testResetSoft(GitConnectionFactory connectionFactory) throws Exception {
        ...
        connection.commit(CommitParams.create().withMessage("add file"));
        //when
        connection.reset(ResetParams.create().withCommit("HEAD^").withType(ResetRequest.ResetType.SOFT));
        //then
        assertEquals(connection.log(LogParams.create()).getCommits().get(0).getMessage(), initMessage);
    ...
    public void testResetMixed(GitConnectionFactory connectionFactory) throws Exception {
        ...
        connection.reset(ResetParams.create().withCommit("HEAD^").withType(ResetRequest.ResetType.MIXED));
        //then
        assertEquals(connection.log(LogParams.create()).getCommits().get(0).getMessage(), initMessage);
        assertTrue(aaa.exists());
